...
1. Open QuizLoop with Gemma missing and show the setup journey.
...
- Public repo that includes `QuizLoop.xcodeproj`, source, README, and demo instructions.
...
The only `.git` repository is inside `QuizLoop/`, while `QuizLoop.xcodeproj` and the root README are at the project root. Before submission, move git to `/Users/suryapolina/Desktop/kaggle` or relocate the Xcode project into the tracked repository so judges can clone and build the app.
